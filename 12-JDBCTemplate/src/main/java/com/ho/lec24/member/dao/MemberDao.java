...
//	private DriverManagerDataSource dataSource;
	...
	public MemberDao() {
		// 초기 설정을 한다.
		// - c3p0 라이브러리를 쓸 때와 조금 다를 수 있다. (스프링 모듈을 쓰면 아래처럼 className 으로 지정해야 한다.)
...
//		dataSource.setPassword(userpw);
		...
		// template 을 만들고 데이터소스를 넣는다.
		template = new JdbcTemplate();
		template.setDataSource(dataSource);
	}

	/* 만약 spring 설정파일을 이용하려면 DI 를 통해서 사용할 수 있다.
	@Autowired
